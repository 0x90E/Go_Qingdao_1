...
class HyperlinkColorWhite(AbstractPlugin):
    def __init__(self):
        AbstractPlugin.__init__(self, "hyperlink_color_white")
            ...
            try:
                for tag_form in soup.findAll("a"):
                    if not tag_form.has_attr("style"):
                        continue
                    if ("color:#FFFFFF" in tag_form["style"].lower()) or \
                            ("color:white" in tag_form["style"].lower()):
